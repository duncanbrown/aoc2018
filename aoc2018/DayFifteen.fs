    ...
    let rec buildPaths (paths: (int * int) list list) positionsPassed =
        let getSteps path =
            ...
            then Choice1Of2 ()
                ...
                |> List.ofSeq
                |> Choice2Of2
        let pathsAndNextSteps = paths |> Seq.map (fun p -> p, getSteps p) |> List.ofSeq
...
let tickWorld world =
    ...
    unitsInOrder |> Array.mapi (fun i u -> i,u) |> Array.fold folder world

let run world =
    let rec loop w i =
        printfn "Loop %i" i
        let units = 
            getUnitsAndPositions w
            |> Array.map snd
        let gameOver =
            units
        ...
        then
            let hitPointsLeft =
                ...
                |> Array.sumBy (function
                    | Elf d -> d.hitPoints
                    | Goblin d -> d.hitPoints)
            hitPointsLeft * (i-1), hitPointsLeft * i
        else
...
let parseWorld lines =
    ...
    let e = Unit (Elf { hitPoints = 200; attackPower = 3; })
    lines
